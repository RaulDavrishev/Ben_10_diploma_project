...
public class Area_3 : MonoBehaviour{
    ...
    public GameObject GhostfreakLevel;

    public GameObject Ghostfreak_Level_Position;
    ...
    IEnumerator TimeDelay(){
        yield return new WaitForSeconds(1.0f);
            ...
            GhostfreakLevel.SetActive(true);
            for(int i=0; i<=10; i++){
                if(Character_models[i].activeSelf){
                    Character_models[i].SetActive(false);
                    Character_models[i].transform.position = Ghostfreak_Level_Position.transform.position;
