...
public class Area_1 : MonoBehaviour{
    ...
    public GameObject Diamondhead_Level_Position;
    ...
    IEnumerator TimeDelay(){
        yield return new WaitForSeconds(1.0f);
            ...
            DiamondheadLevel.SetActive(true);
            for(int i=0; i<=10; i++){
                if(Character_models[i].activeSelf){
                    Character_models[i].SetActive(false);
                    Character_models[i].transform.position = Diamondhead_Level_Position.transform.position;
