...
public class Area_6 : MonoBehaviour{
    ...
    public GameObject Rath_Level_Position;
    ...
    IEnumerator TimeDelay(){
        yield return new WaitForSeconds(1.0f);
            ...
            RathLevel.SetActive(true);
            for(int i=0; i<=10; i++){
                if(Character_models[i].activeSelf){
                    Character_models[i].SetActive(false);
                    Character_models[i].transform.position = Rath_Level_Position.transform.position;
